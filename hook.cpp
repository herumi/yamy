...
bool notify(void *i_data, size_t i_dataSize)
{
	...
	if (g.m_hMailslot != INVALID_HANDLE_VALUE) {
		BOOL ret;
		ret = WriteFile(g.m_hMailslot, i_data, static_cast<DWORD>(i_dataSize), &len, NULL);
#ifndef NDEBUG
		if (ret == 0) {
		...
		if (g.m_hwndTaskTray == 0 || cd.dwData == Notify::Type_threadDetach)
			return false;
		if (!SendMessageTimeout(dwordToHwnd(g.m_hwndTaskTray),
