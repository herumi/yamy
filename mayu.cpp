	...
	BOOL notifyHandler(COPYDATASTRUCT *cd) {
		...
		case Notify::Type_name: {
			...
			if (n->m_type == Notify::Type_setFocus)
				m_engine.setFocus(dwordToHwnd(n->m_hwnd), n->m_threadId,
			...
			bool isMDI = true;
			HWND hwnd = getToplevelWindow(dwordToHwnd(n->m_hwnd), &isMDI);
			RECT rc;
			if (isMDI) {
				...
				<< std::endl;
				hwnd = getToplevelWindow(dwordToHwnd(n->m_hwnd), NULL);
			}
...
public:
	...
	Mayu(HANDLE i_mutex)
		...
		CHECK_FALSE( installMessageHook(hwndToDword(m_hwndTaskTray)) );
					 ...
					 LPTSTR /* i_lpszCmdLine */, int /* i_nCmdShow */)
{
	...
	if (GetLastError() == ERROR_ALREADY_EXISTS) {
		...
		if (g_hookData) {
			...
			PostMessage(dwordToHwnd(g_hookData->m_hwndTaskTray),
